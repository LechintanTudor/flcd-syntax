    ...
    def load_from_file(self, filename):
        with open(filename, 'rt') as f:
            ...
            for non_terminal in self._non_terminals:
                ...
                self._productions[non_terminal] = json_data['nonterminals'][non_terminal]
...
class AugmentedGrammar(Grammar):
    def __init__(self, grammar: Grammar):
        ...
        self._non_terminals = grammar.get_non_terminals()
        self._productions = {}
        for non_term in self._non_terminals:
            self._productions[non_term] = []
            for prod in grammar.get_productions()[non_term]:
                self._productions[non_term].append([Dot()] + prod)

        # for nt in self.productions:
        #     print(nt, self.productions[nt])
